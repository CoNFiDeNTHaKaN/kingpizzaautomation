<?php
 ...
 // wstesting : sandbox_LpoghlOm7rruxBsN2yrfuiPhT4Bg78SzI2nYUMsf
 // ogf live : live_yqkmQAHPMmo8QHRBfE1NN0IEA5UrTBg3updcQMtM

  $goCardless = new \GoCardlessPro\Client([
      'access_token' => 'live_yqkmQAHPMmo8QHRBfE1NN0IEA5UrTBg3updcQMtM',
      'environment' => \GoCardlessPro\Environment::LIVE,
  ]);
  $webhookReturn = "https://server.websitesuccess.co.uk/our-game/webhook";
  ...
  if (isset($_GET['dev'])) {
      if ($_GET['dev'] == 'true') {
          ...
          $webhookReturn = "https://server.websitesuccess.co.uk/our-game/webhook?dev=true";
      }
  }

  $redirectDb = (new MongoDB\Client)->db->redirects;
  $completionRedirect = "https://www.ourgamefootball.com/confirmation-of-purchase";
  ...
  function createRedirect()
  {
      ...
      setcookie('ourGameSession', $cookiehash);
...
XML;
        ...
        !empty($customerGoCardless['Subscription'])) {
          ...
          if ($goCardlessMandateStatus !== "failed" && $goCardlessMandateStatus !== "cancelled" && $goCardlessMandateStatus !== "expired") {
            ...
            "params" => [
              ...
              $interval = (stripos($redirectInfo['description'], 'club') !== false) ? "yearly" : "monthly";
  ...
  function catchWebhookAndUpdateSecureZone()
  {
      ...
      $redirectComplete = $goCardless->redirectFlows()->complete($_GET['redirect_flow_id'], [
        "params" => [
          "session_token" => $_COOKIE['ourGameSession']
        ]
      ]);
      ...
      foreach ($CrmForms as $CrmForm) {
          if ((string) $CrmForm->formName[0] === "GoCardless") {
              ...
              foreach ($crmFormFields as $crmFormField) {
                  ...
                  } elseif ((string) $crmFormField->fieldName === "Mandate") {
                      $crmFormField->fieldValue = $mandate;
                  } elseif ((string) $crmFormField->fieldName === "Subscription") {
...
XML;
      ...
      header(("Location:$completionRedirect?s=s"));
  }

  function efusionSoapy($xmltosend)
  {
      $soap_listretrieve = "https://ourgamefootball.worldsecuresystems.com/catalystwebservice/catalystcrmwebservice.asmx";
      $curlobj = curl_init();
      $headers = array(
